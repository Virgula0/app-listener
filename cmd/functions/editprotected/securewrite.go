...
// writeWithin writes data to dest (mode 0600) so it resolves strictly inside root without
// traversing any symlink below root; missing intermediate directories are created (0700). Atomic
// (temp file in the target dir + rename).
//
// The symlink-safe writer for the non-interactive --put path: os.WriteFile/os.MkdirAll follow
// symlinks, so a symlink anywhere in a guarded tree (a dotfile manager's ~/.ssh/config, a planted
// link) would redirect the write outside every guard, where the daemon imposes no constraint. Every
// component below root is opened O_NOFOLLOW; a symlink is refused. The interactive editor enforces
// the same (internal/tui writeFileKeepMeta).
func writeWithin(root, dest string, data []byte) (err error) {
	...
	// Anchor at root: it is the operator-supplied, daemon-validated watch path, so following a
	// symlink for root itself is fine (the guard is on the real inode); every component BELOW must
	// not be a symlink.
...
// refuseExistingSymlink errors when name already exists as a symlink under dirFD (renameat would
// replace it, but silently swapping an operator's symlink for a file is a surprise; the interactive
// editor refuses too). Missing is fine.
...
// descendNoFollow opens each component in comps relative to dirFD with O_NOFOLLOW, creating missing
// directories (0700), and returns the last directory's fd. A symlink component is refused. The
// returned fd is always the deepest one held (or the input on early error), so one deferred Close
// covers both paths.
