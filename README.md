...
- **live** — when an *edit-protected password* was chosen during `sudo app-listener install` **and** the daemon is running: `edit-protected` authenticates to the daemon over a local root-only control socket (`/run/app-listener-daemon.control`), the daemon briefly grants write access to that one resource, you edit, and the grant is dropped. The daemon keeps running and the fscrypt vaults are never touched. Grants are one-at-a-time, capped at 30 min, and revoked on disconnect or a SIGHUP reload; the socket locks out after 5 failed attempts.
