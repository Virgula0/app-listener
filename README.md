...
- **Binary replacement**: a whitelisted binary at a user-writable path may be modified only by a binary of a resource that whitelists it (never a shell, interpreter, `git`, `cp`, …). A new inode at a whitelisted path is re-admitted only if one of those updaters created it; anything else stays denied (`not re-admitted`) until a reload.
