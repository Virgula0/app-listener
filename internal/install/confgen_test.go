...
// TestGenerateSectionsHasNoHeaderAndAppends verifies GenerateSections emits
// only the [watch] blocks (no file header) and that concatenating them onto
// an existing config yields a file the daemon parser accepts, with the old
// section preserved verbatim (install --diff-catalog).
func TestGenerateSectionsHasNoHeaderAndAppends(t *testing.T) {
	...
	sec := GenerateSections([]Section{{Path: newDir, Allow: []BinaryRule{{Path: "/usr/bin/y"}}, Encrypt: true}})
	if strings.Contains(sec, "# Generated by") {
		t.Errorf("GenerateSections must not emit the file header:\n%s", sec)
	}
	if !strings.Contains(sec, "[watch "+newDir+"]") {
		t.Errorf("GenerateSections missing the new section:\n%s", sec)
	}

	merged := old + sec + "\n"
	if merged[:len(old)] != old {
		t.Fatal("existing config not preserved verbatim")
	}
	cfg := mustParse(t, merged)
	if len(cfg.Resources) != 2 {
		t.Fatalf("merged resources = %d, want 2", len(cfg.Resources))
	}
	if cfg.Resources[0].NeedEncryption || !cfg.Resources[1].NeedEncryption {
		t.Errorf("encryption flags not as expected: %+v", cfg.Resources)
	}
}
