...
// <path>`, `lib_dir <path>` and `lib_binary <path>` (trimmed) inside a
// section body. They are
...
func findSectionEnd(lines []string, start int) int {
	for i := start + 1; i < len(lines); i++ {
		// ANY header ends the body — [libraries] blocks too. Stopping only at
		// "[watch " let the last watch section swallow a trailing library
		// block, so a whitelist refresh would have rewritten its lines.
